...
### Rättat
- Uppdateringen över BLE gick aldrig igång. Förberedelsen raderade hela
  mottagarpartitionen i ett svep, inne i GATT-skrivningens callback och alltså
  på NimBLE:s egen tråd. ATT-svaret kunde inte gå iväg förrän raderingen var
  klar, så appen fick aldrig sitt kvitto, och radering med avstängd cache
  svalt BLE-stacken tills noden startade om i den gamla bilden. Mätt
  2026-09-13: två försök, noll rader från noden, omstart efter trettioåtta
  sekunder. Raderingen sker nu sektor för sektor under skrivningarna.

## 1.1.1 — 2026-09-13

### Rättat
- En ny prenumerant får hela ögonblicksbilden. Bryggan känner varje ID:s senaste
  värde, men prenumerationen nollställde flaggorna och lät appen vänta på nästa
  ändring. En stillastående motorcykel ändrar nästan ingenting: mätt 2026-09-13
  kände bryggan tjugofyra ID:n medan appen fick två.
- Telemetrin kastas inte längre för att den är gammal. Åldersgränsen på två
  sekunder hörde hemma hos reglagen, där ett gammalt knapptryck är fel att
  utföra, men ett telemetrivärde är fortfarande det senaste fordonet sagt och
  paketet bär sin egen tidsstämpel. Dessutom nollställdes flaggan före provet,
  så ett kastat värde var borta för gott.

## 1.1.0 — 2026-09-13

### Notifieringar från telefonen
- Noden läser iPhonens notifieringar över ANCS och visar dem på instrumentet.
  Verifierat mot hårdvara: ett iMessage gav kategori `socialt`, klass 4, och
  innehållet hämtades över Control Point.
- iOS frågar om notisdelning först när enheten **ber** om tjänsten i ett
  solicitation-fält. Scan response bär därför ANCS i stället för nodens eget
  tjänste-UUID; appen matchar på namn och hittar bryggan ändå.
- Godkännandet kommer efter att vi frågat, så upptäckten görs om var tjugonde
  sekund i tio minuter. Ett "Tillåt" tio sekunder in får därmed verkan.
- Den befintliga notifieringslistan, som iOS spelar upp vid varje anslutning,
  hämtas inte och loggas som en summering. Mätt 2026-09-13: hundratre notiser
  på sex sekunder, och en rad per notis konkurrerade med fordonstrafiken.

### Rättat
- Notifieringsbitarnas ordning. Lägre nummer ligger i högre bitar, vilket
  `NOTIFICATIONS.md` mätte mot instrumentet men koden inte följde: klass 4
  hamnade i bit 7..6, som är notifiering 1 — inkommande samtal. Ett SMS såg
  därför ut som att hojen ringde.
- Text kodas med en riktig UTF-8-läsare. Den gamla tog en byte i taget, vilket
  fungerar för ASCII och gör obegriplig sorg av å, ä och ö.
- ANCS- och OTA-rader passerar loggfiltret. De bar ingen version och föll
  därför igenom, vilket gjorde felsökningen blind just där den behövdes.
